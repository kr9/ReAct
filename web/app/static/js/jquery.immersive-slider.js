...
!function($){
  ...
  $.fn.transformSlider = function(settings, pos) {
    ...
    switch(settings.animation) {
      ...
      break;
      case 'fade':
        el.addClass("no-animation").fadeOut("fast", function() {
          el.css({
            "-webkit-transform": "translate3d(" + pos + "%, 0, 0)", 
            "-moz-transform": "translate3d(" + pos + "%, 0, 0)", 
            "-ms-transform": "translate3d(" + pos + "%, 0, 0)", 
   ...
   $(".is-prev").click(function() {
    ...
    $.fn.moveNext = function() {
      var el = $(this),
		...
		//custom
		 if(page_index_number == 15){ 
		 $(".repeat-next").addClass("active");}
		 else {
			  $(".repeat-prev").removeClass("active");
			}
		if(page_index_number == 16){ 
		 $(".repeat-prev").addClass("active");
		 $(".repeat-next").removeClass("active");}
		 
		 if(page_index_number == 2){ 
		 $(".repeat-prev").removeClass("active");}
		 //custom	
    ...
    $.fn.movePrev = function() {
      ...
      var el = $(this),
        ...
        page_index_number = parseInt($(this).find(".is-slide.active").attr("id").replace('slide_','')) - 1;
	//custom
	 if(page_index_number == 14){
		 $(".repeat-prev").removeClass("active");
		 $(".repeat-next").removeClass("active");}
		
	 if(page_index_number == 1){ 
		 $(".repeat-prev").addClass("active");}
		
	 if(page_index_number == 0){ 
		 $(".repeat-next").addClass("active");
		  $(".repeat-prev").removeClass("active");}	
	//custom
