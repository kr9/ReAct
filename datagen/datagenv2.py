...
#   -d <number of days to produce activities for starting from today going back>
#
##############################################################################################
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", help="number of days to produce data for")
    args = parser.parse_args()

    fake = Factory.create()
    start_user_id = int(args.sid)
    end_user_id = int(args.eid)
    number_of_days = int(args.d)
    ...
    for user_id in range(start_user_id, end_user_id):
        ...
        user_file = open("{0:08d}".format(user_id), 'w')
        events = []
